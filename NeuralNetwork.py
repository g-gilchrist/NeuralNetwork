...
def preprocess(raw_text, seq_length):
    ...
    n_vocab = len(chars) # length of chars utilized in the text 
    print(f"Total Characters: {n_chars}") # outputs to console the amount of charachters in the document
    print(f"Total Vocab: {n_vocab}") # outputs to console the amount of unique charachters used in the document

    # prepare the dataset of input to output pairs encoded as integers
    dataX = [] # declares a list to store sequence in digits 
    dataY = [] # declares a list to store sequence out digits
    # this splits up the text into sequences of 100 charachters
    for i in range(0, n_chars - seq_length, 1): # creates a beiginning point of 0 and an end point of the number of charachters misun the sequence length, it increments the for loop by one.
...
def checkpoint(X, y, epochs, batch_size): # passing in the variables created in preprocessing to train the neural network 
    ...
    filepath="weights-improvement-{epoch:02d}-{loss:.4f}-bigger.hdf5"  # Creates a unique filename
    checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1, save_best_only=True, mode='min') # saves the model after each epoch with the unique filename
    callbacks_list = [checkpoint] # creates a list of checkpoints
    
    # fit the model
    # epochs = number of passes, batch size = amount of samples
    model.fit(X, y, epochs, batch_size, callbacks=callbacks_list)


#-----------------------------------------------
# Long Short Term Memory Algorithm
#-----------------------------------------------
# This utilizes the training algorithm to train the nerual network for patterns
...
def lstm(X,y,dataX,int_to_char):
    ...
    filename = "weights-improvement-20-1.4267-bigger.hdf5" # stores the 20th epoch into the filename variable
    model.load_weights(filename) # loads the 20th epoch of our LTSM hdf5 file into our model
    model.compile(loss='categorical_crossentropy', optimizer='adam') # defines the loss function, optimizers, and metrics necessary for predictions
    
    # pick a random seed
    start = numpy.random.randint(0, len(dataX)-1) # creates a random number
    pattern = dataX[start] # the random number becomes the index
    print("\n\n------------------------------------------------------------------------")
    print("\n\nSeed: \n")
...
def txtGen(pattern, n_vocab, model, int_to_char):  
    ...
    print("\n\nGenerated Text: \n")
    # generate characters
    for i in range(1000): # generates 1000 charachter predictions
    	x = numpy.reshape(pattern, (1, len(pattern), 1)) # returns an array with the same data in a different dimension, in this case 1
...
def main():
    ...
    #checkpoint(X,y,20,64) # Only use this when you want to train the neural network
